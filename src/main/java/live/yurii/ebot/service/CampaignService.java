...
public class CampaignService {
  ...
  public void collectCampaignData(Integer campaignId) {
    ...
    for (BattleConsoleDTO.RoundInfoDTO roundInfo : consoleData.getRounds()) {
      ...
      for (int page = 1; page <= consoleData.getMaxPages(); page++) {
        log.info("Fetching page {} of {} for round {}", page, consoleData.getMaxPages(), roundInfo.getId());
        ...
        log.info("Page {}: Found {} contributions", page, pageData.getContributions().size());
    ...
    log.info("Data collection completed for campaign ID: {}", campaignId);
  }
  ...
  private Round createNewRound(Campaign campaign, BattleConsoleDTO.RoundInfoDTO roundInfo) {
    ...
    Round round = Round.builder()
      ...
      .division(division)
      .startedAt(java.time.Instant.ofEpochSecond(roundInfo.getCreatedAt()))
      .campaign(campaign)
      .invaderScore(0)
      .defenderScore(0)
